...
if (!function_exists('base64_to_image')) {
    function base64_to_image($base64_string)
    {
        ...
        $mime_type = finfo_buffer(finfo_open(), $bin, FILEINFO_MIME_TYPE); // extract mime type
        $ext = $mime_type == 'video/3gpp' ? "3gp" : ""; // extract extension from mime type
        ...
        if ($ext != "3gp") {
            ...
            if (!in_array($ext, ['png', 'gif', 'jpeg', 'jpg', '3gp', 'mp3'])) {
        ...
        $img_file = IMAGE_UPLOAD . $file_name;
...
if (!function_exists('send_push_notification')) {
    function send_push_notification($notificationData = [])
    {
        ...
        $fields = array(
            "registration_ids" => json_decode($notificationData['device_token']),
            "notification" => array(
...
function countTokens($amount, $tokenValue)
{
    $tokens = $amount * $tokenValue;
    return $tokens;
}
...
if (!function_exists('point2point_distance')) {
    function point2point_distance($lat1, $lon1, $lat2, $lon2, $unit = 'K')
    {
        if (($lat1 == $lat2) && ($lon1 == $lon2)) {
            ...
            return substr((string)$miles, 0, 7);
        }
    }
}
